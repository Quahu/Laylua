...
-- test:test()");
    ...
    public static void DumpStack(Lua lua, string? location = null)
    {
        DumpStack(lua, Console.WriteLine, location);
    }

    public static void DumpStack(Lua lua, Action<string, object[]> writer, string? location = null)
    {
        var L = lua.State.L;
        var top = lua_gettop(L);
